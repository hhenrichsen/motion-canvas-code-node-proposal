...
describe('CodeRange', () => {
  test('should detect ranges', () => {
    expect(
      isPointInCodeRange([50, 50], pointToPoint(0, 0, 100, 100)[0]),
      'Should work within both ranges',
    ).toBeTruthy();
    expect(
      isPointInCodeRange([50, 1000], pointToPoint(0, 0, 100, 100)[0]),
      'Should accept any column when within lines',
    ).toBeTruthy();
    expect(
      isPointInCodeRange([50, 50], pointToPoint(0, 0, Infinity, Infinity)[0]),
      'Should work with infinity',
    ).toBeTruthy();
    expect(
      isPointInCodeRange([100, 100], pointToPoint(0, 0, 100, 100)[0]),
